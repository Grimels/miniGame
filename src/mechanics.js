...
import { Weapon, Player, Arena } from './gameFile';
...
for (let i = 0; i < arsenal.length; i += 1) {
  const option = arsenal[i];
...
function selectweapon1(name) {
  ...
  const weapon = arsenal.filter(w => w.name === name)[0];
  console.log(weapon);
...
document.getElementById('mainStart').addEventListener('click', start);
document
  .getElementById('weapon1Select')
  .addEventListener('change', () => selectweapon1(document.getElementById('weapon1Select').value));
document
  .getElementById('weapon2Select')
  .addEventListener('change', () => selectweapon2(document.getElementById('weapon2Select').value));
