...
class Arena {
  ...
  combat(p1, p2) {
    if (p1.hp > 0) {
      const damage = p1.attack(p2).toFixed(0);
      this.logger.add(
        `[${combatDateFormatter.format(Date.now())}] ${p1.name} attacks a ${
          p2.name
        } with the damage: ${damage <= 0.001 ? 'MISS' : damage} \t ${p2.name} hp left: ${p2.hp}`,
        'blue',
      );
      if (p2.hp < 0.001) {
        this.logger.add(
  ...
  fight() {
    ...
    this.ids.id1 = setInterval(() => this.combat(this.p1, this.p2), this.p1.stats.speed);

    this.ids.id2 = setInterval(() => this.combat(this.p2, this.p1), this.p2.stats.speed);
  }
}

class DefaultCharacter {
  constructor({
    name, age, speed, weight, hp, armor,
  }) {
    this.stats = {
      name,
      age,
...
class Player extends DefaultCharacter {
  constructor({
    name, age, speed, weight, armor, hp, weapon,
  }) {
    super({
      name,
      age,
  ...
  weaponStrike() {
    ...
    const defaultDamage = coreanRandomGod * (baseAttack - this.stats.age * 0.5 + this.stats.weight * 0.5);
  ...
  attack(target) {
    const damage = this.weaponStrike() * (1 - target.stats.armor / armorCap);
    target.hp -= damage;
    return damage;
  }
}
...
class Weapon {
  constructor({
    name, damage, critChance, missChance,
  }) {
    this.name = name;
    this.stats = {
      damage,
...
export {
  Weapon, Player, Arena, DefaultCharacter,
};
